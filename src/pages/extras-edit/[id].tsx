...
import { GetBrandsResponse, ExtraStatus } from "../../interfaces";
...
import {
  ...
  getExtraById,
  updateExtraById,
} from "../../api/services/extrasService";
...
const ExtrasEdit = () => {
  ...
  const formik = useFormik<ExtraFormValues>({
    ...
    onSubmit: async (values) => {
      ...
      try {
        await updateExtraById(Number(id), payload);
        setToastMessage("Extra actualizado exitosamente");
        setToastVariant("success");
        setShowToast(true);
      } catch (error: any) {
        console.error("Error actualizando el extra:", error);
        setToastMessage("Error al actualizar el extra");
      ...
      const fetchExtra = async () => {
        try {
          ...
          formik.setValues({
            ...
            status: extra.status,
            brandId: extra.brand.id,
          });
        } catch (error) {
          console.error("Error fetching extra:", error);
          setToastMessage("Error al cargar el extra");
          setToastVariant("danger");
          setShowToast(true);
        }
      };
  ...
  const handleDeleteExtra = async () => {
    try {
      await deleteExtraById(Number(id));
      router.push("/extras-list");
    } catch (error) {
      console.error("Error deleting extra:", error);
      setToastMessage("Error al eliminar el extra");
      setToastVariant("danger");
      setShowToast(true);
    }
  };
  ...
  return (
    <React.Fragment>
      <BreadcrumbItem mainTitle="Extras" subTitle="Editar extra" />
              ...
              <Form onSubmit={formik.handleSubmit}>
                <Form.Group className="mb-3">
                  ...
                  <Form.Control
                    ...
                    name="brandId"
                    value={
                      brands.find((brand) => brand.id === formik.values.brandId)
                        ?.name || ""
                    }
                    disabled={true}
                    readOnly
...
ExtrasEdit.getLayout = (page: ReactElement) => {
