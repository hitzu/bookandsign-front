...
const PhotosList = () => {
  ...
  const handleSearch = useCallback(async () => {
    ...
    if (!token) {
      setError("Ingresa el token del evento");
      setPhotos([]);
      return;
    }
    setLoading(true);
    ...
    try {
      ...
      setError(err?.response?.status === 404 ? "Evento no encontrado" : "Error al cargar las fotos");
      setPhotos([]);
    } finally {
      setLoading(false);
    }
  ...
  return (
    <React.Fragment>
      ...
      <BreadcrumbItem mainTitle="Fotos" subTitle="Listado de fotos" />
      <Row>
        <Col sm={12}>
          <Card className="table-card">
            <Card.Body>
              <div className="d-flex justify-content-between align-items-center p-sm-4 pb-sm-2 flex-wrap gap-2">
                <Form
                ...
                >
                  <InputGroup style={{ maxWidth: 400 }}>
                    <Form.Control
                      type="text"
                      placeholder="Token del evento (UUID)"
                      value={eventToken}
...
PhotosList.getLayout = (page: ReactElement) => {
